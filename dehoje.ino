...
void seguidorDeLinha() {
  // Primeiro verifica condições críticas (vermelho)
  if (corE == "Vermelho" && corD == "Vermelho") {
    parado();
    delay(7000);
    return;
  }
  
  // Em seguida verifica verdes (manobras especiais)
  else if (corE == "Verde") {
    giraVerdeEsquerda();
    return;
  } 
  ...
  // Por último verifica as condições normais de seguimento de linha
  else if (corE == "Preto" && corD == "Preto") {
    motorFrente();
  } 
...
// ================== FUNÇÕES DE GIRO PARA VERDE (CORRIGIDAS) ==================
void giraVerdeEsquerda() {
  Serial.println(">>> INICIANDO GIRO VERDE ESQUERDA");
  
  // Avança um pouco antes de iniciar o giro
  motorFrente();
  delay(300);
  
  unsigned long tempoInicial = millis();
  const unsigned long TEMPO_MAXIMO = 4000; // 4 segundos máximo
  
  // Gira para esquerda buscando a linha
  while (millis() - tempoInicial < TEMPO_MAXIMO) {
    motorEsquerda();
    
    // Fazer leitura dos sensores
    leituraExibicao();
    
    // Verificar se encontrou a linha preta em qualquer sensor
    if (corE == "Preto" || corD == "Preto") {
      Serial.println(">>> LINHA ENCONTRADA - SAINDO DO GIRO");
      
      // Pequeno avanço para garantir que está sobre a linha
      motorFrente();
      delay(200);
      return;
    }
  ...
  // Se chegou aqui, não encontrou a linha no tempo máximo
  Serial.println("!!! TEMPO ESGOTADO NO GIRO VERDE ESQUERDA");
  
  // Estratégia de recuperação
  parado();
  delay(500);
  
  // Dar marcha ré e tentar nova abordagem
  motorTras();
  delay(800);
  parado();
  delay(300);
  
  // Girar para direita (sentido contrário)
  motorDireita();
  delay(1200);
  
  // Avançar e esperar encontrar linha
  motorFrente();
  delay(1000);
}

void giraVerdeDireita() {
  Serial.println(">>> INICIANDO GIRO VERDE DIREITA");
  ...
  // Gira para direita buscando a linha
  while (millis() - tempoInicial < TEMPO_MAXIMO) {
    motorDireita();
  ...
  // Se chegou aqui, não encontrou a linha no tempo máximo
  Serial.println("!!! TEMPO ESGOTADO NO GIRO VERDE DIREITA");
  ...
  // Girar para esquerda (sentido contrário)
  motorEsquerda();
  delay(1200);
